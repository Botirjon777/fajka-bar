...
import { X, Search, Image as ImageIcon, Check, Loader2, RefreshCw } from 'lucide-react';
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div
            ...
            className="w-full max-w-4xl bg-bg-card border border-white/10 rounded-[3rem] overflow-hidden relative z-10 shadow-2xl flex flex-col max-h-[85vh]"
          >
            {/* Header */}
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
              <div>
                <h3 className="text-2xl font-serif font-black italic uppercase tracking-tighter mb-1">Select Asset</h3>
                <p className="text-[10px] text-white/40 uppercase font-black tracking-widest">Available in /public/images</p>
              </div>
              <div className="flex items-center gap-4">
                ...
                <button onClick={onClose} className="p-3 rounded-xl bg-white/5 text-white/20 hover:text-white transition-colors">
                  <X size={24} />
                </button>
              </div>
            </div>
            ...
            <div className="p-6 bg-black/20 border-b border-white/5">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 pl-14 pr-6 text-white focus:outline-hidden focus:border-primary/50 transition-all font-bold text-sm"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              {loading && !refreshing ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p className="text-white/20 uppercase tracking-widest font-black text-[10px]">Scanning Directory...</p>
                </div>
              ) : filteredImages.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4 text-white/20">
                  <ImageIcon size={48} strokeWidth={1} />
                  <p className="uppercase tracking-widest font-black text-[10px]">No images found</p>
                </div>
              ) : (
