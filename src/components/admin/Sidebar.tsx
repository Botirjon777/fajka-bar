...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  const navItems = [
    ...
    { to: '/admin/order', icon: <ListOrdered size={20} />, label: 'Menu Order' },
    { to: '/admin/categories', icon: <Layers size={20} />, label: 'Categories' },
    { to: '/admin/products', icon: <GlassWater size={20} />, label: 'Products' },
  ];


  return (
    <div className="h-full flex flex-col bg-bg-card border-l border-white/5 p-8 relative">
      ...
      <nav className="flex-1 space-y-2">
        {navItems.map((item) => (
          <NavLink
            ...
            onClick={onClose}
            className={({ isActive }) => `
              group flex items-center justify-between px-6 py-5 rounded-2xl transition-all duration-300
              ${isActive
          ...
          >
            ...
            <ChevronRight size={14} className={`transition-transform duration-300 ${onClose ? 'opacity-0' : 'group-hover:translate-x-1'}`} />
          </NavLink>
        ))}
      </nav>
