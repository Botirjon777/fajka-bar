...
export default function Footer() {
  ...
  return (
    <footer id="location" className="bg-black pt-32 pb-12 px-6 border-t border-white/5 relative z-10">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 mb-24">
        
        {/* Brand & Social */}
        <div className="lg:col-span-4">
          <h3 className="text-4xl md:text-5xl font-serif font-black gold-gradient italic mb-6">FAJKABAR.</h3>
          <p className="text-white/40 text-lg font-light leading-relaxed mb-10">
            A sanctuary for shisha enthusiasts in the heart of Warsaw. 
            Elegance in every breath, excellence in every service.
          </p>
        ...
        {/* Location & Map */}
          <div className="flex flex-col gap-8">
            <div className="capitalize">
              <h4 className="text-primary font-black mb-8 uppercase tracking-[0.3em] text-[10px]">Location Info</h4>
              <ul className="space-y-8">
                <li className="flex items-start gap-4 text-white/60 group cursor-pointer hover:text-white transition-colors">
                  ...
                  <div className="text-xl font-serif leading-tight">
                    Żurawia 22, <br />
                    00-515 Warszawa <br />
                    <span className="text-[10px] text-primary/40 uppercase tracking-widest font-black mt-2 inline-block italic">WARSAW CITY CENTER</span>
                  </div>
                </li>
            ...
            <div className="pt-4">
              <h4 className="text-primary font-black mb-6 uppercase tracking-[0.3em] text-[10px]">Opening Hours</h4>
              <div className="space-y-4">
                {hours.map((h) => (
                  <div key={h.day} className="flex justify-between items-center group max-w-xs transition-opacity hover:opacity-100 opacity-60">
                    <span className="text-white/40 uppercase text-[10px] tracking-widest">{h.day}</span>
                    <div className="flex-1 border-b border-white/5 mx-4" />
                    <span className="text-lg font-serif text-white">{h.time}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col justify-end">
            <a 
              ...
              target="_blank" 
              rel="noreferrer"
              className="group relative h-[300px] w-full rounded-[2.5rem] overflow-hidden border border-white/10 glass hover:border-primary/50 transition-all duration-500 overflow-hidden"
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                ...
                <h4 className="text-2xl font-serif text-white mb-2 italic">Open in Google Maps</h4>
                <p className="text-white/40 text-[10px] uppercase tracking-[0.4em] font-black">GET DIRECTIONS</p>
              </div>
      ...
      <div className="max-w-7xl mx-auto pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] uppercase tracking-[0.4em] text-white/20">
        ...
        <span className="font-serif italic text-primary/40 underline decoration-primary/20 underline-offset-4">ŻURAWIA 22, 00-515 WARSZAWA</span>
      </div>
    </footer>
  );
}
