      ...
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
          ...
          <div className="flex gap-4 flex-1">
             <div className="relative flex-1 sm:w-64">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 pl-14 pr-6 text-[10px] font-black tracking-widest uppercase focus:border-primary/50 transition-all outline-hidden"
                />
             </div>
             ...
             <div className="relative">
                ...
                <select
                  ...
                  className="bg-white/5 border border-white/5 rounded-2xl py-4 pl-14 pr-10 text-[10px] font-black tracking-widest uppercase appearance-none cursor-pointer outline-hidden focus:border-primary/50"
                >
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-8 py-4 bg-primary text-black font-black uppercase text-[10px] tracking-widest rounded-2xl hover:brightness-110 transition-all shadow-lg shadow-primary/20"
          >
            <Plus size={16} />
            New Product
          </button>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-6">
                  <span className="text-xs font-black uppercase tracking-[0.4em] text-primary shrink-0">{cat.title.en}</span>
                    ...
                    <React.Fragment key={sub._id}>
                      <div className="col-span-full mt-4 mb-2">
                         <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-6 flex items-center gap-3">
                           <span className="w-1 h-1 rounded-full bg-primary" /> {sub.title.en}
                         </h4>
                      </div>
                      {sub.products.map(p => (
                        <ProductListItem 
...
function ProductListItem({ product, onEdit, onDelete }: { product: any, onEdit: () => void, onDelete: () => void }) {
  return (
    <div className="group p-6 bg-bg-card border border-white/5 hover:border-white/10 rounded-[2.5rem] transition-all flex items-center justify-between">
      <div className="flex items-center gap-6 flex-1">
         {product.image ? (
           <div className="w-16 h-16 rounded-2xl border border-white/5 overflow-hidden shrink-0">
              <img src={product.image} className="w-full h-full object-cover opacity-40 grayscale group-hover:opacity-100 group-hover:grayscale-0 transition-all duration-500" />
           </div>
         ) : (
         ...
         <div>
            <h4 className="text-lg font-bold group-hover:text-primary transition-colors">{product.name.en}</h4>
      ...
      <div className="flex items-center gap-3">
         <button onClick={onEdit} className="p-4 rounded-2xl bg-white/5 border border-white/5 text-white/20 hover:bg-white hover:text-black transition-all duration-300">
           <Edit3 size={18} />
         </button>
         <button onClick={onDelete} className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 text-red-500/50 hover:bg-red-500 hover:text-white transition-all duration-300">
           <Trash2 size={18} />
         </button>
      </div>
    </div>
